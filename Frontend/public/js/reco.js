document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!userId || !token) {
    recommendationDiv.innerHTML = `
      <p class="text-red-500">🔒 Please log in to view your personalized recommendation.</p>
    `;
    return;
  }
    ...
    if (!response.ok || !data.generated_recommendation) {
      recommendationDiv.innerHTML = `
        <div class="bg-yellow-100 text-yellow-700 p-4 rounded">
          ⚠ ${data.error || "No recommendation available. Make sure you've submitted both a diary entry and completed the quiz."}
        </div>
      `;
      return;
    }
